...
eight destination sites is closest to their problem, and gets them there. `ml2dl` became
the eighth on 2026-09-25 and carries the NEW sticker; `gaba_excites` became the seventh on
2026-09-23. `draughtsman` became the sixth
on 2026-09-04, when its GitHub Pages certificate finally issued; before that its card pointed
at the repository.
...
|---|---|
| [ml2dl.tonydefazio.com](https://ml2dl.tonydefazio.com) | ml2dl — deep learning built up from its parts, on models small enough to see every number. A development preview; **its repository is private.** **Added 2026-09-25, carries the NEW sticker** |
| [gaba.tonydefazio.com](https://gaba.tonydefazio.com) | gaba_excites — current-clamp excitability simulator: step families, rheobase, F–I, and a movable GABA reversal. Added 2026-09-23 |
| [lookedright.tonydefazio.com](https://lookedright.tonydefazio.com) | It Looked Right — four challenges in working with coding agents, for researchers. Ships out of [`short-course`](https://github.com/syncytium2/short-course). Added 2026-08-28 |
...
no JavaScript, sets no cookies, and carries no analytics** — **seven of the eight destination
sites hold the same posture** (on the no-fonts, no-external-requests half: `gaba_excites` is a
JavaScript app, but its CSP forbids any off-origin request), and `draughtsman`'s is the exception: it requests a webfont
...
| ml2dl | `#A3246A` | assigned here — ml2dl's own site is neutral black on white, and its link blue is too close to no_peak's |
| gaba_excites | `#B5461A` | its `--orange`, darkened to 5.5:1 on white — its `--accent` is a blue too close to no_peak's |
...
The page makes claims about the eight projects. Each was checked against the source
repository on 2026-08-25 — the fifth on 2026-08-31, the sixth on 2026-09-02/03, the seventh on
2026-09-23, the eighth on 2026-09-25 — not written from memory.
...
- **The eight `Born` stamps.** Each is the destination repository's **first commit**, read from
  `git log --reverse` on 2026-09-03 (gaba_excites on 2026-09-23, ml2dl on 2026-09-25): ml2dl `4c7ecae` 2026-09-23 · gaba_excites `c134eaf` 2026-09-21 · draughtsman `ddc332f` 2026-09-01 · short-course `5a6ffc4`
...
- **The ml2dl card**, checked against `origin/main` (`1b85963`) and the live site on
  2026-09-25. The sentence paraphrases the site's own opening ("Deep learning, built up from
  its parts and knocked down again. Each part is introduced only when the model without it
  visibly fails, and every model is small enough to see every number in it").
  - **The repository is private**, so the card carries no repository link, the footer's source
    list omits it, its JSON-LD entry has no `codeRepository`, and "What these have in common"
    says "Seven of the eight repositories are public … ml2dl's is not yet". **When it goes
    public, all four change together.**
  - `development preview` — the site's own banner: "Under construction: a development preview
    of ml2dl. Nothing here is finished." Its pages also carry `noindex, nofollow`; a link from
    here does not override that. Drop the chip when the banner goes.
  - `TF Playground, no analytics` — "our copy of TensorFlow Playground, with its analytics and
    outside fonts removed".
  - The shared claims hold: the site's CSP is `default-src 'none'`, so it can make no request
    at all, and the repository carries a `CLAUDE.md`.
  - **The figure is ml2dl's own computation**: `ga_ml2dl_sim.py` in the darkroom imports
    `ml2dl` and writes `ga-ml2dl-data.json`, using the XOR blobs and seeds of its `neuron_xor`
    and `relu_two_layer_xor` lessons. Best single line 75% (the trained neuron 50%), two ReLU
    hidden units 100%, matching those lessons' own docstrings. 12 of each blob's 50 points are
    drawn.
- **The gaba_excites card**, checked against `origin/main` (`e065498`) and the live site on
  2026-09-23. The sentence paraphrases the site's own og:description ("GABA that hyperpolarizes,
  shunts, or excites depending on where its reversal sits").
  - `2 cell models` — the Pospischil 2008 regular-spiking pyramidal cell and the Adams 2018 GnRH
    neuron, per `gaba_excites/README.md`.
  - `NEURON-checked pyramidal` — **the pyramidal cell only.** The GnRH model has no public
    code; it is checked against the paper's reported firing instead. The chip names the cell so it
    cannot be read as covering both.
  - **The figure is gaba_excites' own simulation**, not a drawing: `ga_gaba_sim.ts` in the
    darkroom imports `gaba_excites/src/core` and writes `ga-gaba-data.json`, which
    `make_figures.py` draws. Published pyramidal cell, one 600 pA step (100–600 ms), the same
    200 Hz / 3 nS / 0.5–10 ms clock-like GABA in every panel; only E_GABA moves. Spike counts
    0 / 0 / 4 at −80 / −60 / −35 mV, and 1 with no GABA — so the −60 mV panel is the README's
    "depolarizes the cell — and still inhibits" row, shown against a dashed resting line.
...
- **"each research-tool repository carries an instruction file written for an agent"** — in
  ...
  the category rather than the filename for exactly this reason. `gaba_excites` carries both.
